...
Why can $Q$ be replaced by $A$? Because $V^\pi(s)$ is fixed while we average the possible actions at
that state. For any fixed $s$:
...
The baseline contribution under the outer trajectory expectation is:
...
This is the missing return trip. We first prove that the baseline vanishes while holding one state
fixed. Then the tower property averages those zero-valued state buckets using how often full
trajectories visit each state.
