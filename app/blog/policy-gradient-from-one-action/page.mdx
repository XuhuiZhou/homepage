...
Let $d_t^\pi$ be the distribution of states reached at time $t$ by full trajectories from the policy.
The baseline contribution under the outer trajectory expectation is zero too. Here is the proof in
slow motion:

1. **Move the sum outside.** Expectation is linear, so we can handle one timestep at a time.
2. **Regroup trajectories by their state.** At time $t$, a full trajectory first determines
   $s_t\sim d_t^\pi$; conditional on that state, its current action has distribution
   $a_t\sim\pi(\cdot\mid s_t)$. This is the tower property, not a different data sampler.
3. **Freeze the state inside the inner average.** Once $s_t$ is fixed, $V^\pi(s_t)$ is a constant and
   can be pulled in front of the action expectation.
4. **Use the local result.** The mean score over possible actions is zero:
   $\mathbb{E}[Z_t\mid s_t]=0$.
5. **Average the state buckets.** Every state contributes zero, so any weighted average of those
   buckets is still zero.
...
For example, if full trajectories reach our two-action state $80\%$ of the time, that state contributes
$0.8\times 0=0$ to the outer average. Another state might be visited $20\%$ of the time, but its own
action branches cancel inside its bucket as well. The outer trajectory expectation is just a weighted
average of these zeros.

This is the missing return trip: we temporarily zoom into the actions available at one fixed state to
prove the cancellation, then use the state-visitation probabilities induced by complete trajectories
to put all the state buckets back together.
