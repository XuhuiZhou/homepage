...
### Why does $H_t$ suddenly appear?

It would be tempting to say, "the past is a constant, so delete it." That is not quite legal. Across
different sampled trajectories, the agent may arrive at time $t$ through different states, actions,
and rewards. The past return is therefore still a random variable in the outer expectation.

We need a smaller experiment where the past really is fixed. Define

$$
H_t=(s_1,a_1,r_1,\ldots,s_{t-1},a_{t-1},r_{t-1},s_t).
$$

$H_t$ is everything observed **immediately before** sampling $a_t$. It includes the current state
$s_t$, but not the current action or anything after it. In an autoregressive language model, $H_t$ is
the prompt plus all previously generated tokens; that prefix determines the state from which the next
token is sampled.

Conditioning on $H_t$ is like pausing the game at one exact checkpoint. Among repeated copies of that
paused situation, the history, current state, and old coins are identical. The only thing we resample
right now is $a_t$. This makes the past reward prefix a genuine constant inside the conditional
expectation:
...
Let the score of the current action be

$$
Z_t=\nabla_\theta\log\pi_\theta(a_t\mid s_t).
$$

First apply the tower property: average within each fixed-history bucket, then average over buckets.

$$
\mathbb{E}[\text{Past}_t Z_t]
=
\mathbb{E}_{H_t}
\left[
\mathbb{E}[\text{Past}_t Z_t\mid H_t]
\right].
$$

Inside one bucket, $\text{Past}_t$ is fixed and can be pulled out. The current action is still sampled
from $\pi_\theta(\cdot\mid s_t)$, so:
...
The cancellation does **not** say that every action has zero score. It says that, at a fixed history,
the probability-weighted average score over all possible current actions is zero. Multiplying every
choice by the same already-earned reward cannot favor one choice over another.

Since the inner conditional expectation is zero for every possible $H_t$, its outer expectation is
also zero:
...
Now the replacement is explicit:
