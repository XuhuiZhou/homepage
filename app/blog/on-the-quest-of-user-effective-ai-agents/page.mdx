import { RLCurvePlot, ModelComparisonChart, StatefulSWEChart, CostEfficiencyChart, VaguePromptChart, InteractionDynamicsChart } from './ChartComponents'
...
We start by applying PPP training to a 36B open-source model `Seed-OSS-36B-Instruct`, producing `PPP-36B`. This model not only outperforms GPT-5 on two real-world complex tasks, but also shows much stronger proactivity—asking clarifying questions when needed, and better alignment with users’ personalized preferences. The results are exciting, and so are the insights we’ve gained:
...
We found that frontier LLMs perform unevenly. For example, GPT-5 scores high on productivity (64.28 F1 on SWE) but much lower on proactivity (35.62) and personalization (41.73). This supports our hypothesis that optimizing only for task success doesn’t create agents that truly work well with users.

Our PPP approach makes big gains: it improves GPT-5’s average score by +16.89 on SWE-Func-Loc<Cite id="swe-func-loc">This is the same task as the SWE-Bench-Verified benchmark, here, we only ask the agent to locate the function needed to fix the issue rather than the full task.</Cite> and +27.78 on BrowseComp+<Cite id="browsecompp">[BrowseComp+](https://arxiv.org/abs/2508.06600) is a QA benchmark replicating the feature of BrowseComp, coming with a static database without the need to search the dynamic, real-world websites.</Cite>. The boosts are especially large for proactivity (+42.62 on SWE) and personalization (+30.42 on SWE).

We also confirmed that all three objectives are essential. Removing any one of them clearly hurts performance. While there are some trade-offs—optimizing only productivity + proactivity lowers personalization (38.92), and optimizing only productivity + personalization reduces proactivity (45.71), we found that the full PPP setup balances all three effectively.

> What are some key interactive behaviors that emerge from PPP training?

We first found that PPP-trained agents learned to distinguish precise vs. vague prompts: they ask questions in 96.8% of vague SWE instances but only 12.4% of precise ones—an 84.4 percentage point difference. This confirms the agent learned to identify ambiguity rather than asking indiscriminately. A critical capability for minimally-disruptive agents is asking only when necessary.

We also discovered the &ldquo;increase-then-decrease&rdquo; learning dynamic. <Ref fig="learning-dynamic" /> reveals a fascinating pattern in how agents learn to ask *better* questions over training.

<Figure id="learning-dynamic">
<InteractionDynamicsChart />
</Figure>

From <Ref fig="learning-dynamic" />, we can see that the PPP method shows a clear increase-then-decrease pattern for medium-effort questions: they rise from 0.14 to 0.35 (as the agent learns to ask more), then fall to 0.15 (as it learns to ask better). Low-effort questions consistently increase from 0.32 to 1.02. In stark contrast, the Prod-only baseline shows continuous degradation: medium-effort questions explode to 0.94 and high-effort to 0.32. This represents an agent becoming &ldquo;lazy&rdquo;—offloading work to the user rather than doing its own exploration.

Finally, to test generalization, we evaluate `PPP-36B` on 8 unseen user preferences (e.g., language: Italian, multilingual, ALL CAPS; formatting: JSON, no commas, three sentences; style: humor, code snippets). We find that `PPP-36B` generalizes well, achieving 76% personalization accuracy, only slightly below the 79.46% on seen preferences. In contrast, if we only train the model with task success reward, the performance collapses from 90% to 34%.


## Looking forward

This blog post was a collaboration between the authors and the very AI agents we study. The process was... interesting. We felt both the frustration of their flaws and the undeniable power of their speed, a speed no human developer can match. Despite their imperfections, one thing is clear: there is no going back. *Now* leaves us to think about how can we push the boundary of AI agents to be more user-effective and truly collaborative? Here, we share our thougths and open questions, hoping to inspire new research in this direction. We split the discussion into three critical aspects of reinforcement learning: *context*, *priors*, and *environment*.

### More human user context
Probably the quickest fix is to bridge the *context gap*. Most collaborations break down because agents are blind to the huge chunk of context users have just &ldquo;below the surface&rdquo; of an instruction. This context comes from *various places*: the website we just browsed, the paper we just read, the people we just talked to, the food we just ate, etc. If we *squeeze hard enough*, we can *get* more human user context. For example, we can capture the user's screen as visual context<Cite id="usercontext">[Shaikh et al., 2025](https://arxiv.org/abs/2505.10831)</Cite> or manage conversational memory to understand intent over time<Cite id="memgpt">[MemGPT](https://arxiv.org/abs/2310.08560)</Cite>.
...
However, it is still unclear that how the quality of the user simulator could influence the trained agent's interaction behavior. Can we find tasks where models trained on bad user simulators could make a significant difference in the agent's interaction behavior? And how are we going to judge whether a user simulator is good enough?
