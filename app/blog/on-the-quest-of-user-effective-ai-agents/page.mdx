...
We also confirmed that all three objectives are essential. Removing any one of them clearly hurts performance. While there are some trade-offs—optimizing only productivity + proactivity lowers personalization (-21.82), and optimizing only productivity + personalization reduces proactivity (-5.95), we found that the full PPP setup balances all three effectively.

> What are some key interactive behaviors that emerge from PPP training?

We first found that PPP-trained agents learned to distinguish precise vs. vague prompts: they ask questions in 100% of vague SWE instances but only 6% of precise ones—an 94 percentage point difference. This confirms the agent learned to identify ambiguity rather than asking indiscriminately. A critical capability for minimally-disruptive agents is asking only when necessary.
...
From <Ref fig="learning-dynamic" />, we can see that the PPP method shows a clear increase-then-decrease pattern for medium-effort questions: they rise from 0.13 to 0.38 (as the agent learns to ask more), then fall to 0.04 (as it learns to ask better). Low-effort questions consistently increase from 0.35 to 0.92. In stark contrast, the Prod-only baseline shows continuous degradation: medium-effort questions explode to 0.53 and high-effort to 0.31. This represents an agent becoming &ldquo;lazy&rdquo;—offloading work to the user rather than doing its own exploration.
