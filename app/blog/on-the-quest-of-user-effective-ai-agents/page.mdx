...
Probably the quickest fix is to bridge the *context gap*. Most collaborations break down because agents are blind to the huge chunk of context users have just "below the surface" of an instruction. This context comes from *various places*: the website we just browsed, the paper we just read, the people we just talked to, or things we just remembered.

And what's interesting is, if we *squeeze hard enough*, we can *get* more human user context. We can capture the user's screen as visual context <Cite id="usercontext">[Shaikh et al., 2025](https://arxiv.org/abs/2505.10831)</Cite> or manage conversational memory to understand intent over time <Cite id="memgpt">[MemGPT](https://arxiv.org/abs/2310.08560)</Cite>.

These are just the first steps. The true future direction isn't just capturing *individual* streams. The new frontier is what [Hua, Ye, Fu, et al., 2025](https://arxiv.org/abs/2510.26493) call **"Context Engineering 2.0"**. This next phase is about understanding the **"context of context"**—the challenge of dynamically fusing all these disparate, multi-modal sources (our screen, our voice, our application state, and our history) into a single, unified model of our **"confluence of contexts"** <Cite id="contextengineering2">[Hua, Ye, Fu, et al., 2025](https://arxiv.org/abs/2510.26493)</Cite>.
